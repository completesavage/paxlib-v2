...
class PolarisAPI {
    ...
    public function getPatronCheckouts($patronId) {
        ...
        if ($result['ok'] && isset($result['data']['PatronItemsOutGetRows'])) {
            ...
            // Count DVD checkouts only. Polaris installations can expose the material
            // description under different field names, so inspect material/format fields plus
            // collection/call-number metadata. Do not count every checked-out item.
            $dvdCount = 0;
            foreach ($items as $item) {
                $dvdFields = [
                    $item['FormatDescription'] ?? '',
                    $item['MaterialTypeDescription'] ?? '',
                    $item['MaterialType'] ?? '',
                    $item['MaterialTypeName'] ?? '',
                    $item['CollectionName'] ?? '',
                    $item['CollectionAbbreviation'] ?? '',
                    $item['CallNumber'] ?? ''
                ];

                $dvdMetadata = strtolower(implode(' ', array_filter(array_map('strval', $dvdFields))));
                if (preg_match('/\bdvd\b/i', $dvdMetadata)) {
                    $dvdCount++;
                }
            }
