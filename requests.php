...
if ($method === 'POST') {
    ...
    // Enforce the DVD limit on the server too. A Get Now request must have a
    // real patron barcode, and Polaris must confirm fewer than 5 DVDs are out.
    if ($type === 'now') {
        $patronBarcode = trim((string)($patron['barcode'] ?? ''));
        if ($patronBarcode === '') {
            http_response_code(403);
            echo json_encode([
                'ok' => false,
                'error' => 'Library card sign-in required to check out DVDs'
            ]);
            exit;
        }
        ...
        try {
            ...
            if (!$patronResult['ok'] || empty($patronResult['data']['PatronID'])) {
                http_response_code(403);
                echo json_encode(['ok' => false, 'error' => 'Unable to verify patron account']);
                exit;
            }

            $checkoutResult = $api->getPatronCheckouts($patronResult['data']['PatronID']);
            if (!$checkoutResult['ok']) {
                http_response_code(503);
                echo json_encode(['ok' => false, 'error' => 'Unable to verify current DVD checkouts']);
                exit;
            }

            $dvdCount = (int)($checkoutResult['dvdCheckouts'] ?? 0);
            if ($dvdCount >= 5) {
                http_response_code(403);
                echo json_encode([
                    'ok' => false,
                    'error' => 'DVD checkout limit reached (5 DVDs)',
                    'dvdCheckouts' => $dvdCount
                ]);
                exit;
            }
        } catch (Throwable $e) {
            error_log('requests.php DVD limit verification failed: ' . $e->getMessage());
            http_response_code(503);
            echo json_encode(['ok' => false, 'error' => 'Unable to verify current DVD checkouts']);
            exit;
        }
